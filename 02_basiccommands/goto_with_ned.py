...
def send_reverseThrust(ned): #current NED
    ...
    msg = vehicle.message_factory.set_position_target_local_ned_encode(
        ...
        north,east,down,  # x, y, z velocity in m/s Applies reverse thrust on X wrt drone body (not sure if it works if yaw isn't working yet!
...
def send_reverseThrust2(): #current NED
    print("Reverse thrust")
    msg = vehicle.message_factory.set_position_target_local_ned_encode(
        0,  # time_boot_ms (not used)
        0, 0,  # target system, target component
...
startingPos = point_on_circle(radius,angle, center.lat, center.lon)
firstTargetPosition = LocationGlobalRelative(startingPos.lat, startingPos.lon, 10)
fly_to(vehicle,firstTargetPosition,10)
angle = 3
while angle <= 360:
    print ("\n Flying angle: " + str(angle))
    nextTarget = (point_on_circle(radius, angle, center.lat, center.lon))

    #print ("Flying from LAT: " + str(currentLocation.lat) + " to " + str(nextTarget.lat))
    #print ("Flying from LON: " + str(currentLocation.lon) + " to " + str(nextTarget.lon))
    ...
    ned = setNED(currentLocation, nextTarget)
    ...
    while distance_to_target > 1:
        ...
        print ("Target  Lat: " + str(nextTarget.lat) + " Target  lon: " + str(nextTarget.lon))
        print ("Angle: " + str(angle) + " Dist: " + str(distance_to_target) + "  NED: " + str(ned.north) + " " + str(ned.east) + " " + str(ned.down))
    ...
    #    send_reverseThrust(ned)  #Check if this is the ned set in the loop
    #    print("Velocity: " + str(vehicle.groundspeed))

    print("Next target:", angle, nextTarget.lat, nextTarget.lon)
    angle = angle + 40
...
while angle <=360:
    ...
    lat2_array.append([])
    lon2_array.append([])
    lat2_array[-1].append(point.lat)
    lon2_array[-1].append(point.lon)
    angle = angle +2
